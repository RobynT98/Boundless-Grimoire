...
export default function Library() {
  ...
  return (
    <div className="p-4 space-y-4">
        ...
        {collections.map(c => {
          ...
          return (
            <Link
              ...
              key={c.id}
              className="card p-4 hover:brightness-110 transition focus:outline-none focus:ring-2"
            ...
            >
              ...
              {recent.length > 0 && (
                <div className="mt-3 space-y-1">
                  {recent.map(e => (
                    <div key={e.id} className="text-xs text-main truncate">• {e.title}</div>
                  ))}
                </div>
              )}

              <div className="mt-3 text-[11px]" style={{ color: 'var(--accent)' }}>
                Öppna →
              </div>
            </Link>
          )
      ...
      {entries.length === 0 && (
        <p className="text-muted">Inga poster ännu. Skapa din första via <b>Skapa</b>.</p>
      )}
    </div>
  )
}